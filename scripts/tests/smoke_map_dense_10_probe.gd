...
func _initialize() -> void:
	...
	if failures.is_empty():
		print("dense 10 map probe smoke passed")
		quit(0)
	else:
...
func _check_catalog() -> void:
	var nodes := EncounterCatalog.map_nodes("dense_10")
	if nodes.size() != 10:
		failures.append("dense map should expose exactly 10 visible nodes")
	if EncounterCatalog.final_step("dense_10") != 7:
		failures.append("dense map final step should be 7")
	var step_one := EncounterCatalog.available_node_types(1, "dense_10")
	if not step_one.has("event") or not step_one.has("elite"):
		failures.append("dense map step one should expose event/elite branch")
	var step_three := EncounterCatalog.available_node_types(3, "dense_10")
	if not step_three.has("combat") or not step_three.has("event"):
		failures.append("dense map step three should expose combat/event branch")
...
func _check_scene() -> void:
	...
	if nodes.size() != 10 or buttons.size() != 10:
		failures.append("dense map scene did not build 10 nodes and buttons")
	...
	for i in range(nodes.size()):
		...
		rects.append(rect)
		if button == null:
			failures.append("dense map button " + str(i) + " missing")
			continue
		if button.position.distance_to(rect.position) > 0.01:
			failures.append("dense map button " + str(i) + " position mismatch")
		if button.size.distance_to(rect.size) > 0.01:
			failures.append("dense map button " + str(i) + " size mismatch")
		var should_be_enabled := int(node["node_index"]) == 3
		if button.disabled == should_be_enabled:
			failures.append("dense map button " + str(i) + " disabled state mismatch")
		if rect.position.x < 78.0 or rect.end.x > 1204.0 or rect.position.y < 88.0 or rect.end.y > 634.0:
			failures.append("dense map card outside table bounds: " + str(node.get("node_id", "")))
	for a in range(rects.size()):
		for b in range(a + 1, rects.size()):
			if rects[a].intersects(rects[b]):
				failures.append("dense map card rects overlap: " + str(a) + " and " + str(b))
	run_map.queue_free()
	await process_frame
