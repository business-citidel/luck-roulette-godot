...
const CHARACTER_IDS := ["default_guard_dice"]
...
func _initialize() -> void:
	for character_id in CHARACTER_IDS:
		...
		for i in range(RUN_COUNT):
			runs.append(_simulate_run("numeric-floor1-guard-" + str(i).pad_zeros(4), character_id))
		var summary: Dictionary = _summarize(runs, character_id)
		print("NUMERIC_FLOOR1_GUARD_SUMMARY " + JSON.stringify(summary))
	quit(0)
...
func _simulate_run(seed_text: String, character_id: String) -> Dictionary:
	...
	rng.seed = hash("numeric-floor1-guard:" + seed_text)
