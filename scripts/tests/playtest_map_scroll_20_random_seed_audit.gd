...
func _initialize() -> void:
	...
	for i in range(10):
		...
		await _show_seed(seed_text, i)
	if active_scene != null:
		active_scene.queue_free()
		await process_frame
	if failures.is_empty():
		print("scroll 20 random seed audit playtest passed")
		quit(0)
	else:
...
func _show_seed(seed_text: String, index: int) -> void:
	if active_scene != null:
		active_scene.queue_free()
		active_scene = null
		await process_frame
	...
	await _settle(18)
	await _shot(str(index).pad_zeros(2) + "_" + seed_text)
