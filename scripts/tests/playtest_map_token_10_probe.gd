...
func _initialize() -> void:
	...
	await _show_map("00_token_10_start", 0, [])
	await _show_map("01_token_10_first_branch", 1, ["d0"])
	await _show_map("02_token_10_mid_branch", 3, ["d0", "d1", "d2"])
	await _show_map("03_token_10_rest_current", 4, ["d0", "d1", "d2", "d3"])
	await _show_map("04_token_10_late_shop", 6, ["d0", "d1", "d2", "d3", "d4", "d5"])
	await _show_map("05_token_10_boss_gate", 7, ["d0", "d1", "d2", "d3", "d4", "d5", "d6"])

	if active_scene != null:
		active_scene.queue_free()
		await process_frame
	if failures.is_empty():
		print("token 10 map probe playtest passed")
		quit(0)
	else:
